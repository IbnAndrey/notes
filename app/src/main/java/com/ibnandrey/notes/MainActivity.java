...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AtomicReference<FirebaseUser> firebaseUser= new AtomicReference<>(firebaseAuth.getCurrentUser());

        if(firebaseUser.get() !=null)
        {
            finish();
        ...
        mlogin.setOnClickListener(v -> {
            ...
            else
            {
                ...
                firebaseAuth.signInWithEmailAndPassword(mail,password).addOnCompleteListener(task -> {
                    ...
                    {
                        firebaseUser.set(firebaseAuth.getCurrentUser());

                        if(firebaseUser.get().isEmailVerified()==true)
                        {
                            finish();
                            startActivity(new Intent(MainActivity.this,notesactivity.class));
                        }
                        else
                        {
                            ...
                            Toast.makeText(getApplicationContext(),"Підтвердіть Email",Toast.LENGTH_SHORT).show();
                            firebaseAuth.signOut();
                        }
                    }
                    else
                    {
                        Toast.makeText(getApplicationContext(),"Невірний Email або пароль",Toast.LENGTH_SHORT).show();
                        mprogressbarofmainactivity.setVisibility(View.INVISIBLE);
                    }
